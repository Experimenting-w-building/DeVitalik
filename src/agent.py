...
class ZerePyAgent:
    def __init__(self, agent_name: str):
        try:
            ...
            has_discord_tasks = any(
                "discord" in task["name"] for task in agent_dict.get("tasks", [])
            )
            discord_config = next(
                (
                    config
            ...
            if has_discord_tasks and discord_config:
                ...
                self.discord_message_interval = discord_config[
    ...
    def generate_embeddings(self, chunks):
        ...
        return self.connection_manager.perform_action(
            connection_name="openai", action_name="generate-embeddings", params=[chunks]
        )
